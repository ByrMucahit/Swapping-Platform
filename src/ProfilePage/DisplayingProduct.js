...
products.on("value", function(product){
    if(product.exists())
    {
        ...
        product.forEach(function(singleProduct){
            ...
            productHtml += `<div class="col-md-4 col-sm-6"><div class="small-box-c"><div class="small-img-b"><a href="#" ><span onclick="removed()"  class="close">&times</span><img  id="s" alt="#" src="${eachOneProduct.link}" style="height:100px; height:200px;"/></a></div><div class="dit-t clearfix"><div class="left-ti"><p>${eachOneProduct.ürün_sahibi}</p><br><h4>${eachOneProduct.ürün_ismi}</h4><p><span>${eachOneProduct.kategori}</span></p></div><a href="#" tabindex= "0"><button id="product" value="${eachOneProduct.ürün_ismi}">"Urune Git"</button></a></div></div></div></div>`
